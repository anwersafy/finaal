...
var SheetNames=[
  ...
  Future<String> getCellData(String sheetTitle, int row, int column) async {
    final sheet = await getWorksheetByTitle(sheetTitle);
  ...
  Future<List<List<String>>> getRows(String sheetTitle) async {
    ...
    getColumn(String sheetTitle, int column) async {
      final sheet = await getWorksheetByTitle(sheetTitle);
      return await sheet!.values.column(column);
  }
