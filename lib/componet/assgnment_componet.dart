      ...
      await ExcelHelper.readCell('Assignments',cellindex).then((value) {
        if(value != null) {
          if (j == 0) {
            ...
            GetStorageHelper.writeData('Assign_total_marks_$i', value);
          }
          if (j == 1) {
            Assign1_total_controller[i].text = value;
            GetStorageHelper.writeData('Assign1_total_$i', value);
          }
          if (j == 2) {
            Assign2_total_controller[i].text = value;
            GetStorageHelper.writeData('Assign2_total_$i', value);
          }
          if (j == 3) {
            Assign3_total_controller[i].text = value;
            GetStorageHelper.writeData('Assign3_total_$i', value);
          }
          if (j == 4) {
            Assign4_total_controller[i].text = value;
            GetStorageHelper.writeData('Assign4_total_$i', value);
          }
        }
      });
}
